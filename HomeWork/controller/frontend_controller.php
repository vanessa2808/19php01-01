		...
		function handleRequest() {
			...
			switch ($controller) {
				case 'home':
					...
					include 'view/element_frontend/footer.php';
					break;
				case 'user':
		...
		function handleNew($action){
			switch ($action) {
				case 'list_new':
					...
					include 'view/news/list_news_frontend.php';
					break;
				case 'new_detail':
					$id = $_GET['id'];
					$model = new FrontendModel();
					$newDetail = $model->getnewDetail($id);
					$newDetail = $newDetail->fetch_assoc();
					$commentList = $model->getCommentList($id);
					include 'view/news/list_news_detail.php';
					# code...
					break;
