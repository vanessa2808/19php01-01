		...
		function handleProduct($action) {
			switch ($action) {
				case 'add_product_category':
					if (isset($_POST['add'])) {
					  ...
					  if ($model->addProductCategory($name) === TRUE) {
					  	header("Location: admin.php?controller=product&action=list_product_category");
					  }
					}
				...
				case 'list_product_category':
					$model = new BackendModel();
					$listProductCategory = $model->listProductCategory();
					include 'view/products/list_product_category.php';
					break;
				case 'edit_product_category':
					$id = $_GET['id'];
					$model = new BackendModel();
						$editProductCategory =$model->getProductCategories($id);
						// edit
						if (isset($_POST['editProductCategory'])) {
							$name = $_POST['name'];
						
						if ($model->editProductCategory($id,$name) === TRUE) {
							header("Location: admin.php?controller=product&action=list_product_category");
						}
						}
						include 'view/products/edit_product_category.php';
						break;	
		...
		switch ($action) {
			case 'add_new':
						if (isset($_POST['add_news'])) {
							...
							if ($_FILES['image']['error'] == 0) {
							$image = $_FILES['image']['name'];
							$pathUpload = 'webroot/uploads/news/';
					  ...
					  if ($model->addNews($title, $description, $image, $writer) === TRUE) {
					  	move_uploaded_file($_FILES['image']['tmp_name'], $pathUpload.$image);
					  	header("Location: admin.php?controller=new&action=list_new");
					  }
					}
				...
				$id = $_GET['id'];
					$model = new BackendModel();
						...
						if (isset($_POST['edit_form_news'])) {
						 ...
						 // edit vao database
						if ($model->editNews($id,$title, $description, $image,  $writer) === TRUE) {
							header("Location: admin.php?controller=new&action=list_new");
						}
						}
